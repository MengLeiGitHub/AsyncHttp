...
public class FileDataConvert extends BaseDataConvert<File> {
	...
	private  void  RandomAccessFileDataConvert(BaseHttpRequest<File> request, InputStream input, long len)
			throws IOException, HttpException{
		input.available();
		
		long filelong=request.getResponse().getContentLength();
		
		RandomAccessFile[] randomAccessFiles=new RandomAccessFile[2];
